#include "shell.h"

/**
 * _increment_while_not_null - This function increments a counter while a string is not null
 * @s: This pointer refers to the string to be checked
 * @i: This pointer refers to the counter to be incremented
 *
 * Return: The incremented counter
 */
int _increment_while_not_null(char **s, int *i)
{
    /* use loop */
    while (*(*s)++)
        (*i)++;

    /* This function returns the incremented counter */
    return (*i);
}

/**
 * _strlen - This function returns the length of a string
 * @s: This pointer refers to the string whose length to check
 *
 * Return: The length of the string
 */
int _strlen(char *s)
{
    int i = 0;

    /* This function returns the length of the string */
    return ((!s) ? 0 : _increment_while_not_null(&s, &i));
}

/**
 * compare_chars - This function compares two characters
 * @c1: The first character
 * @c2: The second character
 *
 * Return: The result of the comparison
 */
int compare_chars(char c1, char c2)
{
    /* use if */
    if (c1 != c2)
        return (c1 - c2);
    else if (c1 == c2)
        return (0);
    else
        return (c1 < c2 ? -1 : 1);
}

/**
 * _strcmp - This function performs lexicographic comparison of two strings
 * @s1: This pointer refers to the first string
 * @s2: This pointer refers to the second string
 *
 * Return: Negative if s1 < s2, positive if s1 > s2, zero if s1 == s2
 */
int _strcmp(char *s1, char *s2)
{
    int result = 0;

    /* use loop */
    do {
        result = compare_chars(*s1, *s2);
        /* use if */
        if (result != 0)
            /* This function returns the result of the comparison */
            return (result);
        s1++;
        s2++;
    } while (*s1 && *s2);
    /* This function returns the result of the comparison */
    return (compare_chars(*s1, *s2));
}

/**
 * check_chars - This function checks if two characters are equal
 * @needle: This pointer refers to the first character
 * @haystack: This pointer refers to the second character
 *
 * Return: 1 if the characters are equal, 0 otherwise
 */
int check_chars(const char **needle, const char **haystack)
{
    /* This function returns the result of the check */
    return ((*(*needle)++ == *(*haystack)++) ? 1 : 0);
}

/**
 * starts_with - This function checks if a string starts with a substring
 * @haystack: This pointer refers to the string to search
 * @needle: This pointer refers to the substring to find
 *
 * Return: Address of next character of haystack or NULL
 */
char *starts_with(const char *haystack, const char *needle)
{
    const char *temp_needle = needle;
    const char *temp_haystack = haystack;

    /* use loop */
    do {
        /* use if */
        if (!check_chars(&temp_needle, &temp_haystack))
            /* This function returns NULL if the substring is not found */
            return (NULL);
    } while (*temp_needle);

    /* This function returns the address of the next character of haystack */
    return ((char *)temp_haystack);
}

/**
 * find_end - This function finds the end of a string
 * @str: This pointer refers to the string to find the end of
 *
 * Return: Pointer to the end of the string
 */
char *find_end(char *str)
{
    char *end = str;

    /* use loop */
    for (; *end; ++end);
    /* This function returns a pointer to the end of the string */
    return (end);
}

/**
 * copy_str - This function copies a string to another string
 * @dest: This pointer refers to the destination string
 * @src: This pointer refers to the source string
 */
void copy_str(char *dest, char *src)
{
    /* use loop */
    for (; *src; ++src, ++dest)
        *dest = *src;
    *dest = '\0';
}

/**
 * _strcat - This function concatenates two strings
 * @dest: This pointer refers to the destination string
 * @src: This pointer refers to the source string
 *
 * Return: Pointer to the destination string
 */
char *_strcat(char *dest, char *src)
{
    char *ret = dest;
    char *end_of_dest = find_end(dest);

    copy_str(end_of_dest, src);
    /* This function returns a pointer to the destination string */
    return (ret);
}
